...
(function () {
  ...
  var trainerPresets = ['עדכנתי לך את התפריט — תיכנס לטאב תזונה', 'תזכור לעדכן משקל היום', 'כל הכבוד על השבוע 💪'];
  var traineePresets = ['תודה! הבנתי את ההנחיות 💪', 'סיימתי את הצהריים, היה טעים', 'אפשר להזיז את הפגישה לשבוע הבא?'];
  var WEIGHTS = [79.0, 78.4, 77.9, 77.3, 76.8];                       // שקילה שבועית, כמו שהמאמן ביקש
  var WDATES = ['05/08', '12/08', '19/08', '26/08', '02/09'];
  ...
  function initial() {
    return {
      chat: [
        ...
        { from: 'trainee', text: 'בסדר גמור, רק פספסתי את האימון של שלישי', time: '11:25' },
        { from: 'trainer', card: true, time: '13:55' }                       // הצעת פגישה מהמאמן — ממתינה לתשובת המתאמן
      ],
      unreadByTrainee: 1, unreadByTrainer: 1, ronUnread: 1,
      meals: [
        meal('ארוחת בוקר', '07:30–08:30', [
          grp('חלבון', 'בחר אחד:', [['מעדן חלבון', 150, 15, 10, 3, 130], ['טונה במים', 150, 30, 0, 2, 140], ['חביתה מ־3 ביצים', 150, 20, 1, 16, 230], ['גבינה לבנה 5%', 150, 17, 5, 7, 150]]),
        ...
        meal('צהריים', '12:30–13:30', [
          ...
          grp('פחמימה', 'בחר אחד:', [['אורז מבושל', 200, 5, 56, 1, 260], ['פסטה ללא גלוטן', 180, 6, 55, 2, 265], ['תפוח אדמה מבושל', 220, 4, 40, 0, 180], ['בטטה אפויה', 220, 4, 46, 0, 200]]),
          grp('ירקות', 'חופשי — כמה שבא לך:', [['סלט ירקות', 0, 0, 0, 0, 0]], true)]),
        ...
        meal('ערב', '19:00–20:00', [
          ...
          grp('ירקות', 'חופשי — כמה שבא לך:', [['ירקות מוקפצים', 0, 0, 0, 0, 0]], true)])
      ],
  ...
  var PANES = {
    weight: [{ name: 'משקל גוף', hint: 'המשקל שאתה מזין בעצמך, לאורך הטווח שבחרת', big: function () { return (S.weightLogged ? '76.5' : '76.8') + ' ק"ג'; }, from: 'מ-79.0 ק"ג', vals: function () { return WEIGHTS.concat(S.weightLogged ? [76.5] : []); }, color: '#F07C1A', dates: WDATES, ranges: ['חודש', 'שבועיים', 'שבוע'], weightBtn: true }],
    measures: [
      { name: 'מותן', hint: 'ההיקפים שהמאמן מודד לך — מדד אחד בכל פעם', big: '83.5 ס"מ', from: 'מ-85.0 ס"מ', vals: [85.0, 84.7, 84.4, 84.1, 83.8, 83.5], color: '#12939D', dates: ['30/06', '21/07', '11/08', '01/09'], ranges: ['חודש', 'חודשיים', 'שלושה'] },
      { name: 'ירך', hint: 'ההיקפים שהמאמן מודד לך — מדד אחד בכל פעם', big: '58.8 ס"מ', from: 'מ-59.6 ס"מ', vals: [59.6, 59.4, 59.2, 59.0, 58.8], color: '#12939D', dates: ['14/07', '28/07', '18/08', '01/09'], ranges: ['חודש', 'חודשיים', 'שלושה'] }],
    strength: [
      ...
      { name: 'התקדמות בנפח', hint: 'סך העומס: סטים × חזרות × משקל, בכל התרגילים', big: '4860 ק"ג', from: 'מ-3900 ק"ג', vals: [3900, 4100, 4050, 4300, 4500, 4480, 4700, 4860], color: '#F07C1A', dates: ['12/07', '30/07', '17/08', '03/09'], ranges: ['חודש', 'שבועיים', 'שבוע'], foot: 'לא המשקל שעל המוט. תרגילי משקל גוף וקרדיו (0 ק"ג) לא נספרים' },
  ...
  var trainer = {
    ...
    screens: {
      home: function () {
        ...
        var reqAvs = (S.ronPending ? '<span>' + cav(client('ron'), 46, 'trainee') + '</span>' : '') + '<span>' + cav(client('shira'), 46, 'trainee') + '</span>' + (S.traineeRequestSent ? '<button data-act="go:profile:connection">' + cav(client('daniel'), 46, 'trainee') + '</button>' : '');
      ...
      clients: function (ph) {
        ...
        var list = CLIENTS.filter(function (cl) { return !needsOnly || clientReasons(cl.id).length; }).map(function (cl) {
          ...
          var meeting = cl.id === 'daniel' ? ['21', 'שני', 'בדיקת התקדמות + עדכון תפריט'] : cl.id === 'michal' ? ['14', 'שני', 'לקיחת מידות + תמונות'] : cl.id === 'yossi' ? ['20', 'ראשון', 'עדכון תוכנית אימון'] : null;
      ...
      profile: function (ph) {
        ...
        var hero = '<div class="k-hero"><div class="who">' + av(cl.c, cl.ini, 64, null, 'big') +
          ...
          '<span class="k-goal">חיטוב</span></div></div>';
        ...
        if (tab === 'progress') {
          ...
          var wvine = '<div class="k-vine rest">' + days.map(function (d, i) {
            var g = i === 1 ? 'ok' : i === 2 ? 'miss' : (i === 3 && S.workoutDone ? 'ok' : i > today ? 'fut' : 'ex');
          ...
          body = panelTitle('תזונה ותכנית אימונים') +
            '<div class="k-expand open"><button class="head" data-act="xp:nut"><b>תזונה</b><small>2000 קלוריות ליום</small></button><div class="body"><i class="k-div"></i>' + vine +
              '<div class="k-adapt"><span class="t">' + icon('book', 'bi v-coach') + 'תבנית: תפריט יומי 2000</span><i class="k-div"></i><div class="k-target"><span class="col"><small>היעד</small><b>2000 קק״ל</b></span><i class="vdiv"></i><span class="col"><small>המאכלים נותנים</small><b class="v-coach">1720–2140 קק״ל</b></span></div><i class="k-div"></i><b class="k-good">היעד נמצא בתוך הטווח</b></div>' +
            ...
            '<div class="k-expand' + (woOpen ? ' open' : '') + '"><button class="head" data-act="xp:wo"><b>אימונים</b><small>3 אימונים בשבוע</small></button>' +
              (woOpen ? '<div class="body"><i class="k-div"></i>' + wvine + '<p class="k-missed">פספס אימון אחד השבוע</p>' + '<div class="k-aero">' + icon('run', 'bi v-coach') + '<b' + (S.aerobic >= 3 ? ' class="v-positive"' : '') + '>אירובי · ריצה · ' + S.aerobic + ' מתוך 3 השבוע</b></div>' +
            ...
            panelTitle('היקפים') + '<div class="k-expand' + (ph.openMe ? ' open' : '') + '"><button class="head" data-act="xp:me"><b>היקפים</b><small>2 היקפים</small></button>' +
              (ph.openMe ? '<div class="body"><i class="k-div"></i>' + [['מותן', '83.5 ס"מ', '▼', 'v-positive', '01/09/2026'], ['ירך', '58.8 ס"מ', '▼', 'v-positive', '01/09/2026']].map(function (m) { return '<div class="k-mtrack"><b>' + m[0] + '</b><span class="val"><b class="v-coach">' + m[1] + '</b><i class="' + m[3] + '">' + m[2] + '</i></span><small>מדידה אחרונה: ' + m[4] + '</small><i class="k-div"></i><div class="btns"><button class="k-compact secondary" data-act="noop">' + icon('chart', 'bi') + 'גרף</button><button class="k-compact secondary" data-act="noop">' + icon('history', 'bi') + 'היסטוריה</button></div></div>'; }).join('') + primary('+ הזנת מדידה', 'noop', 'coach') + '</div>' : '') + '</div>' +
          ...
          body = (S.traineeRequestSent ? panelTitle('בקשות') + card('<div class="k-tags">' + statusTag('ממתין לאישורך', 'coach') + '</div>' + heading('עדכון תפריט') + factPair(inline('שעה', '18:00'), inline('משך', '30 דקות')) + '<div class="k-center">' + inline('פלטפורמה', 'זום') + '</div><div class="k-received">התקבלה: 09/09 · ' + now() + '</div>' + '<div class="k-center">' + pill('לפרטי הפגישה', 'event-note', 'sheet:coord2') + '</div>', 'request', 'sheet:coord2') : '') +
            ...
            panelTitle('פגישה חוזרת') + '<div class="k-infobox"><div class="k-row-ic">' + icon('repeat', 'bi v-coach') + '<b>כל שבוע · שני · 10:00 · זום</b></div></div>' +
            ...
            panelTitle('תזכורת קצרה') + '<div class="k-infobox"><small class="k-hint center">הערה פרטית שלך על הלקוח. המתאמן לא רואה אותה.</small><p class="k-center">לוודא עדכון משקל לפני הפגישה ביום שני</p><div class="k-center"><button class="k-compact secondary" data-act="noop">' + icon('edit', 'bi') + 'עריכה</button></div></div>' +
            '<div class="k-gap20"></div>' + primary("לצ'אט", 'go:thread', 'coach');
        } else {
          body = panelTitle('מבנה תשלום') + '<div class="k-infobox"><div class="k-center">' + inline('שם התבנית', 'מנוי סטנדרטי') + '</div>' + factPair(inline('תדירות תשלום', 'כל חודש'), inline('יום החיוב', '15')) + factPair(inline('תשלום הבא', S.danielPaid ? '15/10/2026' : '15/09/2026')) +
          ...
          body = filterToggle(f === 'trainees' ? 'בקשות מתאמנים' : 'בקשות שלי', 'mfilter', f === 'trainees' ? 'trainee' : 'coach') +
            (f === 'trainees'
              ? (S.traineeRequestSent ? requestCard({ cl: client('daniel'), dd: '16', mon: 'ספט׳', status: 'ממתין לאישורך', color: 'coach', topic: 'עדכון תפריט', time: '18:00', dur: 30, platform: 'זום', received: '09/09 · ' + now(), act: 'sheet:coord2' }) : '') +
                ...
                requestCard({ cl: client('shira'), dd: '21', mon: 'ספט׳', status: 'הצעת חלופית - ממתין לאישורך', color: 'coach', topic: 'פגישת היכרות + מדידות ראשוניות', time: '11:00', dur: 60, platform: 'פרונטלי', place: 'הסטודיו, רחוב הרצל 5', received: '08/09 · 14:20', act: 'noop' }) +
      ...
      payments: function (ph) {
        ...
        if (cat === 'month') {
          ...
          body = '<p class="k-context">כל התשלומים שנרשמו, מהאחרון</p>' + search('חפש לפי שם...') + '<div class="k-gap12"></div>' +
            ...
            card(clientHeader(client('or'), '01', 'ספט׳') + '<div class="k-center">' + inline('אמצעי תשלום', 'העברה בנקאית') + '</div><div class="k-center">' + inline('לאיזה חודש', 'ספטמבר 2026') + '</div><div class="k-center">' + pill('היסטוריית תשלומים', 'receipt', 'noop') + '</div>', 'meeting', 'noop') +
            card(clientHeader(client('yossi'), '15', 'אפר׳') + '<div class="k-center">' + inline('אמצעי תשלום', 'אשראי') + '</div><div class="k-center">' + inline('לאיזה חודש', 'אפריל – יוני 2026') + '</div><div class="k-center">' + pill('היסטוריית תשלומים', 'receipt', 'noop') + '</div>', 'meeting', 'noop');
        }
  ...
  var trainee = {
    ...
    screens: {
      ...
      nutrition: function (ph) {
        ...
        return screenTitle('תזונה') + '<p class="k-goalline">חיטוב · 2000 קלוריות</p><div class="k-gap20"></div>' + sectionTitle('הארוחה הבאה') + next +
      ...
      workouts: function (ph) {
        var nextName = S.workoutDone ? 'אימון C – רגליים' : 'אימון A – חזה + כתפיים', nextMin = S.workoutDone ? 62 : 58;
        ...
        var WOS = [['אימון A – חזה + כתפיים', '4 תרגילים', S.workoutDone, [['לחיצת חזה במוט', '4 סטים · 8–12 חזרות'], ['לחיצת כתפיים', '4 סטים · 8–12 חזרות'], ['פרפר בכבלים', '3 סטים · 12–15 חזרות'], ['פלאנק', '3 סטים · 45 שנ׳']]],
          ['אימון B – גב + זרועות', '3 תרגילים', true, [['מתח', '4 סטים · 6–10 חזרות'], ['חתירה בהרכנה', '4 סטים · 8–12 חזרות'], ['כפיפת מרפק', '3 סטים · 10–12 חזרות']]],
      ...
      meetings: function (ph) {
        ...
        if (cat === 'upcoming') {
          ...
          body = banner + card(heading('מדידות + עדכון תפריט') + factPair(inline('תאריך', '10/08/2026'), inline('שעה', '10:00')) + factPair(inline('משך', '60 דקות'), inline('פלטפורמה', 'זום')) + '<div class="k-center">' + pill('לפרטי הפגישה', 'event-note', 'noop') + '</div>', 'meeting pad20', 'noop');
        }
        return screenTitle('פגישות') + bar + '<div class="k-gap12"></div>' + body;
      },
      chat: function (ph) {
  ...
  function fabSpec(ph) {
    ...
    if (t === 'today') {
      ...
      if (nextMeal()) return { icon: 'check', cls: 'v-trainee', act: 'eatnext', label: 'סמן שאכלת' };
      return null;
    }
    if (t === 'nutrition') return nextMeal() ? { icon: 'check', cls: 'v-trainee', act: 'eatnext', label: 'סמן שאכלת' } : { icon: 'minus', cls: 'v-trainee', act: 'uneatlast', label: 'הורד סימון' };
  ...
  function confirmCheck(title, detail) { return '<div class="k-confirm"><span class="ring">' + icon('check', 'bi') + '</span><b>' + title + '</b>' + (detail ? '<small>' + detail + '</small>' : '') + '</div>'; }
  function sheetHtml(kind, ph) {
    ...
    if (kind === 'request') return formSheet('בקשת פגישה', ph.saved ? confirmCheck('הבקשה נשלחה למאמן', '')
      : pickerRow('תאריך', '16/09/2026', 'calendar-today') + '<div class="k-avail">' + icon('event-busy', 'bi v-coach') + '<b>אפשר לבחור רק ימים שהמאמן הגדיר בהם זמינות — שאר הימים חסומים בלוח.</b></div>' + pickerRow('שעה', '18:00', 'schedule') +
        labeled('נושא (חובה)', 'עדכון תפריט', 'על מה תרצה לדבר?') + '<div class="k-two">' + labeled('פלטפורמה', 'זום', '') + labeled('משך הפגישה', '30 דקות', '') + '</div>' + primary('שלח בקשה', 'requestsend', 'trainee'));
    ...
    if (kind === 'meeting') return '<div class="k-detsheet"><b class="ttl">פרטי הפגישה</b>' + heading('בדיקת התקדמות + עדכון תפריט') + factPair(inline('תאריך', '21/09/2026'), inline('שעה', '10:00')) + factPair(inline('משך', '60 דקות'), inline('פלטפורמה', 'זום')) +
      ...
      timeline([['coach', 'event', 'התיאום נפתח ע"י המאמן', '26/08/2026 · 14:02'], ['trainee', 'check-circle', 'אושרה ע"י המתאמן', '26/08/2026 · 18:40']].concat(S.attendance ? [['trainee', 'check-circle', 'המתאמן אישר סופית הגעה', 'היום · ' + now()]] : [])) + '</div>';
    if (kind === 'tmeeting') {
    ...
    if (kind === 'coord') return '<div class="k-detsheet"><b class="ttl">פרטי תיאום</b>' + heading('עדכון תוכנית אימונים') + '<div class="k-tags">' + statusTag('ממתין לאישורך', 'coach') + '</div>' + factPair(inline('תאריך', '17/09/2026'), inline('שעה', '08:30')) + factPair(inline('משך', '30 דקות'), inline('פלטפורמה', 'טלפון')) +
      ...
      timeline([['trainee', 'event', 'התיאום נפתח ע"י המתאמן', '08/09/2026 · 09:40']]) + '</div>';
    if (kind === 'coord2') return '<div class="k-detsheet"><b class="ttl">פרטי תיאום</b>' + heading('עדכון תפריט') + '<div class="k-tags">' + statusTag('ממתין לאישורך', 'coach') + '</div>' + factPair(inline('תאריך', '16/09/2026'), inline('שעה', '18:00')) + factPair(inline('משך', '30 דקות'), inline('פלטפורמה', 'זום')) +
      ...
      timeline([['trainee', 'event', 'התיאום נפתח ע"י המתאמן', '09/09/2026 · ' + now()]]) + '</div>';
  ...
  function syncNav(ph, app) {
    ...
    var fab = fabSpec(ph), el = ph.navwrap.querySelector('.dp-fab:not(.is-gone)');
    if (fab) {
      if (!el) {
        ph.navwrap.insertAdjacentHTML('beforeend', '<button class="dp-fab ' + fab.cls + ' is-new" data-act="' + fab.act + '" aria-label="' + fab.label + '" title="' + fab.label + '">' + icon(fab.icon, 'bi') + '</button>');
        var nel = ph.navwrap.lastElementChild; requestAnimationFrame(function () { requestAnimationFrame(function () { nel.classList.remove('is-new'); }); });
      } else {
        el.className = 'dp-fab ' + fab.cls; el.setAttribute('data-act', fab.act); el.setAttribute('aria-label', fab.label); el.setAttribute('title', fab.label);
        var use = el.querySelector('use'); if (use && use.getAttribute('href') !== '#i-' + fab.icon) use.setAttribute('href', '#i-' + fab.icon);
      }
    } else if (el) { el.classList.add('is-gone'); el.removeAttribute('data-act'); setTimeout(function () { el.remove(); }, 280); }
  ...
  function quiet(ph, fn) {
    ph.root.classList.add('no-anim'); fn(); void ph.root.offsetWidth;
    var free = function () { ph.root.classList.remove('no-anim'); };
    clearTimeout(ph.animT); ph.animT = setTimeout(free, 250);                       // רשת ביטחון: rAF מווסת בלשונית ברקע
    requestAnimationFrame(function () { requestAnimationFrame(function () { clearTimeout(ph.animT); free(); }); });
  }
  function toast(ph, t) {
  ...
  function fitNav(ph) {
    ...
    var on = c.querySelector('.dp-tab.is-on'), navW = c.offsetWidth, pad = 5;
    if (on && navW && on.offsetWidth) {
      var tW = on.offsetWidth, tL = on.offsetParent === c ? on.offsetLeft + c.clientLeft : on.offsetLeft - c.offsetLeft;
  ...
  function flip(ph) {
    ...
    // ההחלפה נקשרת לשעון האנימציה, לא ל-setTimeout: אם פריים נופל, currentTime עדיין מספר את הזווית האמיתית
    var spin = ph.root.getAnimations().filter(function (a) { return a.animationName === 'dp-spin'; })[0];
    var swapped = false;
    var done = function () { ph.root.classList.remove('is-turning'); wrap.classList.remove('is-flipping'); ph.turning = false; };
    if (!spin) { setTimeout(function () { if (!swapped) { swapped = true; swap(); } }, 475); setTimeout(done, 960); return; }
    var watch = function () {
      if (swapped) return;
      if (spin.playState === 'finished' || spin.currentTime >= 470) { swapped = true; swap(); return; }
